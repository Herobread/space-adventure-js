...
export const asciiMap = {
    putSymbol: function (char, x, y) {
        if (x >= 0 && x < window.w && y >= 0 && y < window.h && screen) {
            if (window.renderer.blendMode) {
                let symbol = screen[y][x]
                if (symbol) {
                    if (blendings[symbol] && blendings[symbol][char]) {
                        char = blendings[symbol][char]
                    }
                }
            }
...
export const renderer = {
    ...
    drawTransparentObject: async function (object, x, y) {
        ...
        if (!transperencyMap && window.showUndefinedTransperencyMapWarning) {
            ...
            this.drawObject('no transperency map', x + object.w + 1, y - 1)
        }
    },
