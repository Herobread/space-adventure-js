...
export const animations = {
	...
	animate: async function (
		...
		if (debug)
			console.log(
				`added animation at ${x} ${y}, ts = ${tickSpeed}, ms = ${moveSpeed}, xVel = ${xVel}`
			)
	...
	tick: function () {
		...
		activeAnimations.map((animation, id) => {
			...
			if (animation.stage >= animation.sprites.length) {
				if (debug) console.log(`removed animation ${id}`)
	...
	render: function () {
		...
		activeAnimations.forEach((animation) => {
			renderer.drawObject(
				animation.sprites[animation.stage],
				animation.x,
				animation.y
			)
		})
