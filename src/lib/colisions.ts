...
interface baseColisionRectangle extends rectangle {
	name: string
	[key: string]: any
}
...
export const colisions = {
	checkIfPointInSquare: function (point: basePoint, rectangle: rectangle) {
		let { x, y } = point
		let x1 = Math.floor(x)
		let y1 = Math.floor(y)
	...
	checkRectanglesColision: function (
	...
	addRectangleColision: function (
		object: baseColisionRectangle,
		onColision: Function
	) {
