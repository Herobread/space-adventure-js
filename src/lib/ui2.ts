...
export const ui2 = {
    button: function (button: buttonOptions) {
        ...
        const isHovered = colisions.checkIfPointInSquare({ x: pointer.x, y: pointer.y }, { x: x - 1, y: y - 1, w: width, h: height })
    ...
    dropDown: function (name: string, options: dropDownOption[], x: number, y: number, pointer: any, onOpen: Function = () => { }, onClose: Function = () => { }) {
        ...
        if (openedDropdowns[name]) {
            options.forEach((option, i) => {
                ...
                if (option.split) {
                    ...
                    if (colisions.checkIfPointInSquare({ x: pointer.x, y: pointer.y },
                        {
                            x: x_,
                            y: y_,
        ...
        if (pointer.click) {
            if (colisions.checkIfPointInSquare({ x: pointer.x, y: pointer.y }, { x: x, y: y, w: name.length + 2, h: 2 })) {
                onOpen()
                // console.log('first')
