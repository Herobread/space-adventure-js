...
export class Player {
	...
	handleBullets() {
		this.bullets.map((bullet, i) => {
			...
			animations.animate(
				art.animations.smallParticle,
				bullet.x,
				bullet.y,
				bullet.xVelocity * 0.5 * 0,
				bullet.yVelocity + randomInRange(-0.1, 0.1),
				{
			...
			colisions.addRectangleColision(
				{
					w: 1,
					h: 1,
					x: bullet.x,
	...
	async tick(pointer, keyboard, pad) {
		...
		if (this.hp === 0 && !this.dead) {
			...
			window.formatedScores = scores.forEach((record, i) => {
				res += `${i + 1}. ${record.name} - ${numberWithCommas(
					record.score
				)}\n\n`
			})
		...
		if (window.clock % 3 == 0 && !this.dead) {
			if (this.isTopEngineWorking)
				animations.animate(
					...
					this.x,
					this.y + 0,
					randomInRangeFloat(-2.5, -0.2),
					randomInRangeFloat(-0.2, 0.2),
